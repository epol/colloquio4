...
\begin{frame}{Problemi collegati}
  Analogamente a quanto viene fatto con le distanze, ha senso studiare
  algoritmi per ritornare un'approssimazione di tutti i cammini
  minimi.
  \vfill

  Per esempio un oracolo che ritorna tutti i cammini tra $s$ e $t$ di
  lunghezza minore di $r\delta\pa{s,t}$ (cio\`e una
  $r$-approssimazione) potrebbe occupare meno spazio in memoria di un
  oracolo esatto.
\end{frame}
...
\begin{frame}{Contenuti bonus}
  \begin{itemize}
  \item \hyperref[sec:kcamm]{$k$-cammini pi\`u brevi}
  \item \hyperref[sec:oracolidist]{Oracoli per distanze esatti}
  \item \hyperref[sec:approx]{Oracoli per distanze approsimati}
  \item \hyperref[sec:APSP]{APSP}
  \item \hyperref[sec:oracolocamminipiu]{Miglioramento dell'oracolo
      per cammini}
  \end{itemize}
\end{frame}
